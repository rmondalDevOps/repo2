...
public class TS72_DesignPage_StepSchedulingConfigurationTest_2 extends TestBase{

	CommonFunction cfunction = new CommonFunction();
	String msg = "";

	public void workflow_TS72_DesignPage_StepSchedulingConfigurationTest_2() throws InterruptedException {
		...
		//Click on Diagram in left menu
				...
				try {
					...
					/*SikuliHelp.selectLocationOnMapSikuliClick("TS15_stepName_1024x768");
					cfunction.sync(2);
				...
				cfunction.sync(2);
				msg = "Enter Step name as \"Manual Step A\"";
				try {
				...
				//Open Time drop down list and choose "Hour Of Day"
				msg = "Open Time drop down list and choose \"Hour Of Day\"";
				try {
					Design.selectTimeFromDropDown("Hour of Day");
					CommonFunction.logStatus("PASS", msg);
				} catch (Exception e) {
					e.printStackTrace();
					CommonFunction.logStatus("FAIL", msg);
				...
				cfunction.sync(10);
				//The Hour  of Day is selected. 
				//The default time to run the step is 00:00.
				//Pick a time to run the step by opening the time picker: 12:30
				msg = "Pick a time to run the step by opening the time picker: 12:30";
				try {
					Design.enterTimeInput("12:30");
					cfunction.sync(2);
					Design.clickOnWaitUntilASpecificTimeRadioBtn();
					CommonFunction.logStatus("PASS", msg);
				} catch (Exception e) {
				...
				cfunction.sync(2);
				//Click back on "Start/End" step
				msg = "Click back on \"Start/End\" step";
				try {
					SikuliHelp.selectLocationOnMapSikuliClick("TS15_stepEnd_1366x768");
					CommonFunction.logStatus("PASS", msg);
				} catch (Exception e) {
				...
				/*msg = "Verify the previous edits are saved in the dialog.";
				try {
					Design.clickOnoffsetInput();
					cfunction.sync(50);
					if(Design.verifyInputValueOfMonth("5"))
					{
						CommonFunction.logStatus("PASS", msg);
					}else {
						CommonFunction.logStatus("FAIL", msg);
